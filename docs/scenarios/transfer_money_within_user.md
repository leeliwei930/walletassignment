...
sequenceDiagram
    ...
    Client->>Backend: POST /api/v1/wallet/:wallet_id/transfer<br/>(amount, recipient_user_id, description)
    ...
    Backend->>Backend: Compute updated balance and balance debited on destination wallet
    Backend->>Backend: Compute updated balance and balance to credit from source wallet
    ...
    Backend->>PostgresDB: Update destination wallet account balance
    PostgresDB-->>Backend: Return updated destination wallet account balance
    ...
    Backend->>PostgresDB: Update source wallet account balance
...
sequenceDiagram
    ...
    Client->>Backend: POST /api/v1/wallet/:wallet_id/transfer<br/>(amount, recipient_user_id, description)
    Backend->>Backend: Validate transfer amount
    Backend->>Backend: Validate source and destination users
    Note over Backend: Self-transfer Validation Check
    Backend-->>Client: Return Bad Request<br/>Status: 400<br/>{<br/>error_code: "ERR_TRANSFER_10001",<br/>message: "Cannot transfer to own account"<br/>}
```
...
sequenceDiagram
    ...
    Backend-->>Client: Return Not Found<br/>Status: 404<br/>{<br/>error_code: "ERR_TRANSFER_10002",<br/>message: "Invalid recipient id"<br/>}
```
