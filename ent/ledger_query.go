...
func (lq *LedgerQuery) QueryWallet() *WalletQuery {
	query := (&WalletClient{config: lq.config}).Query()
...
func (lq *LedgerQuery) Clone() *LedgerQuery {
	...
	return &LedgerQuery{
		...
		withWallet: lq.withWallet.Clone(),
...
func (lq *LedgerQuery) WithWallet(opts ...func(*WalletQuery)) *LedgerQuery {
	query := (&WalletClient{config: lq.config}).Query()
...
func (lq *LedgerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ledger, error) {
	...
	if query := lq.withWallet; query != nil {
		if err := lq.loadWallet(ctx, query, nodes, nil,
			func(n *Ledger, e *Wallet) { n.Edges.Wallet = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LedgerQuery) loadWallet(ctx context.Context, query *WalletQuery, nodes []*Ledger, init func(*Ledger), assign func(*Ledger, *Wallet)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Ledger)
	for i := range nodes {
		fk := nodes[i].WalletID
...
func (lq *LedgerQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := lq.ctx.Fields; len(fields) > 0 {
		...
		if lq.withWallet != nil {
			_spec.Node.AddColumnOnce(ledger.FieldWalletID)
		}
	}
